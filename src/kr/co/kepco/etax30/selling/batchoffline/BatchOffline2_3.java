 ...
 * 프로그램 명          : BatchOffline2_3
 * 프로그램 아이디      : BatchOffline2_3.java
 * 프로그램 개요        : XML 생성, 압축 배치작업 실행(main())                                                  
 * 관련 테이블          : 
 * 관련 모듈            : BatchProcess
 * 작성자               : 양형공
 * 작성일자             : 2009-12-31
	...
	public static void main(String[] args) {
		...
		try {
			...
			if(targetMonth.length()==6 && args[1].length()==2){ //두번째 항목은 배치그룹 차수임
				   BatchOffline2_3.OnOffbatch = "OFF";
					now_grp_no = args[1];
			}else{
		...
		if (b1) {
			try {
				bp.makeXML(targetMonth, machine, threadNum, machineNum, now_grp_no);//생성
				bp.compressFiles(targetMonth, machineNum, now_grp_no);//압축
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
